...
@section('title', 'Gerenciar PDFs')

@section('content')
    <h1 class="mt-4 mb-4">Gerenciar PDFs</h1>
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <form action="{{ route('admin.upload') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="arquivo" class="form-label">Selecionar PDF</label>
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">PDFs Enviados</h5>
                    ...
                    <tbody>
                        @foreach ($uploads as $upload)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ asset('storage/' . $upload->path) }}" target="_blank" class="btn btn-sm btn-info">Ver PDF</a>
                                    <form action="{{ route('admin.upload.delete', ['tipo' => 'pdf', 'nomeArquivo' => $upload->generated_name]) }}" 
                                          method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" 
                                                onclick="return confirm('Excluir este PDF?')">Excluir</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
            ...
            @else
                <p class="text-muted">Nenhum PDF enviado ainda.</p>
            @endif
        </div>
    </div>
@endsection