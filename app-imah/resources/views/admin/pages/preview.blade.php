...
@php
    $slideImages = [];
    foreach( $page->slide as $image ){
        $slideImages[] = ['src' => $image , 'alt' => ''];
    }
@endphp
...
@section('content')
    <section class="product-hero product-hero--carousel" aria-labelledby="product-title" data-product-carousel>
        ...
        <div class="hero-detail hero-detail--overlay">
            <article class="product-card product-card--floating">
                ...
                <p class="category">{{ $page->equipament }}</p>
    ...
    <dialog class="image-modal" data-product-modal aria-label="Imagem ampliada">
        ...
        <img src="{{ $slideImages[0]['src'] }}" alt="{{ $slideImages[0]['alt'] }}" data-product-modal-image>
    </dialog>
    ...
    <section class="intro-band" id="descricao">
        <div class="container">
            <div class="intro-copy">
                ...
                @if( isset($page->pdf[0]) )
                    <a class="dark-button" rel="noopener noreferrer" target="_blank" href="{{ url($page->pdf[0]) }}">Baixar catálogo <span aria-hidden="true">↗</span></a>
                @endif
            </div>
