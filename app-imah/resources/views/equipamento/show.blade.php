...
<head>
    ...
    <meta name="description" content="{{ $page->title }} - Indústria de Máquinas IMAH">
    <meta name="keywords" content="máquinas industriais, IMAH, serigrafia, {{ $page->equipment }}">
    <meta property="og:title" content="{{ $page->title }} - Indústria de Máquinas IMAH">
    <meta property="og:description" content="Detalhes sobre {{ $page->title }} da IMAH. Confira especificações e solicite um orçamento.">
    ...
    <title>{{ $page->title }} - Indústria de Máquinas IMAH</title>
    <!-- Bootstrap CSS -->
...
<body>
        ...
        <section class="product-section">
            <div class="container" style="max-width:1200px">
                ...
                @if (!empty($page->equipment))
                    <a href="{{ url('/equipamentos') }}#{{ strtolower(str_replace(['á', 'é', 'í', 'ó', 'ú', 'ã', 'õ', 'ç'], ['a', 'e', 'i', 'o', 'u', 'a', 'o', 'c'], $displayNames[$page->equipment] ?? $page->equipment)) }}" class="equipment-back-link">
                        <i class="bi bi-arrow-left"></i> Voltar para Equipamentos
                    </a>
                @endif
                <!-- Galeria de Imagens -->
                    ...
                    <div class="media-container no-video">
                        <div class="gallery">
                            @foreach ($page->images as $image)
                                @if (file_exists(storage_path('app/public/uploads/imagem/' . $image)))
                                    <img src="{{ asset('storage/uploads/imagem/' . $image) }}" alt="{{ $page->title }} - Imagem" onclick="openLightbox('{{ asset('storage/uploads/imagem/' . $image) }}')">
                                @endif
                            @endforeach
                        </div>
                    </div>
                ...
                <!-- Vídeo (se houver) -->
                @if (!empty($page->video) && file_exists(storage_path('app/public/uploads/video/' . $page->video)))
                    <div class="video-section">
                        <div class="video-wrapper">
                            <video controls preload="metadata">
                                <source src="{{ asset('storage/uploads/video/' . $page->video) }}" type="video/mp4">
                ...
                <div class="button-group mt-3">
                    <a href="https://wa.me/554135576008?text=Olá,%20gostaria%20de%20um%20orçamento%20para%20a%20{{ urlencode($page->title) }}" class="btn-contact" target="_blank">Solicite um Orçamento</a>
                    @if (!empty($page->pdf) && file_exists(storage_path('app/public/uploads/pdf/' . $page->pdf)))
                        <a href="{{ asset('storage/uploads/pdf/' . $page->pdf) }}" class="btn-contact" target="_blank">Catálogo PDF</a>
                    @endif
                    <button class="btn-share" onclick="shareLink('{{ url('/' . $page->slug) }}', '{{ $page->title }}')">Compartilhar</button>
                </div>
            </div>
        </section>
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <ul>
                            ...
                            <li><a href="{{ url('/admin') }}">Reservado</a></li>
                        </ul>
                    </div>
                    <div class="col-md-4">
                        <h5>Siga-nos</h5>
    ...
    <script>
        ...
        // Ajustar margem da product section com base na altura da navbar
        window.addEventListener('load', () => {
            const navbarHeight = document.querySelector('.navbar-custom').offsetHeight;
            document.querySelector('.product-section').style.marginTop = `${navbarHeight}px`;
        });
        ...
        function shareLink(url, title) {
            if (navigator.share) {
                navigator.share({
                    title: title,
                    text: 'Confira esta máquina incrível da IMAH!',
                    url: url
                }).then(() => {
